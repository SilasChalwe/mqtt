...
// Mock sensor instance supplied explicitly to SolarManager.
...
void setup() {
  ...
  // Provide already-read battery and solar state for estimator.
  estimator.update(85.0f, 12.0f, 2.0f, 24.0f);
  estimator.setFilteredSoC(0.85f);

  // Test 2: Available current with battery + solar.
  float available = estimator.getAvailableCurrent(100.0f, 5.0f, 1.0f);
  ...
  float runtime1 = estimator.getEstimatedRuntimeHours(100.0f, 1.0f);
  ...
  float runtime2 = estimator.getEstimatedRuntimeHours(100.0f, 5.0f);
  ...
  float available2 = estimator.getAvailableCurrent(100.0f, 5.0f, 0.0f);
